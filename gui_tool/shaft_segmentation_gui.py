...
try:
    from damage_detection_v2 import run_damage_detection
    HAS_DAMAGE_DETECTION = True
except ImportError:
    HAS_DAMAGE_DETECTION = False
...
class ProcessingThread(QThread if USE_PYQT else threading.Thread):
    ...
    def __init__(self, processor, input_path, output_dir, slice_height=10.0, enable_damage_detection=True):
        if USE_PYQT:
            super().__init__()
        else:
            super().__init__()
        ...
        self.enable_damage_detection = enable_damage_detection
    ...
    def run(self):
        try:
            ...
            if self.enable_damage_detection and HAS_DAMAGE_DETECTION:
                self.emit_progress_value(90)
                self.emit_progress("\n" + "="*50)
                self.emit_progress("Running Damage Detection...")
                self.emit_progress("="*50)

                try:
                    damage_report_dir = output_dir / "damage_report"

                    # Save combined LAS file for damage detection
                    # The damage detection module works on LAS files with classification
                    temp_las_path = output_dir / "temp_classified.las"
                    self.emit_progress("Preparing classified point cloud for analysis...")

                    # Create a temporary LAS file with predictions
                    if HAS_LASPY:
                        # Create new LAS file with classification
                        header = laspy.LasHeader(version="1.4", point_format=0)
                        las_out = laspy.LasData(header)
                        las_out.x = points[:, 0]
                        las_out.y = points[:, 1]
                        las_out.z = points[:, 2]
                        las_out.classification = all_predictions.astype(np.uint8)
                        las_out.write(str(temp_las_path))

                        # Run damage detection
                        self.emit_progress("Analyzing steel members for damage...")
                        damage_report = run_damage_detection(
                            str(temp_las_path),
                            str(damage_report_dir),
                            scan_info=f"Processed from {Path(self.input_path).name}"
                        )

                        # Clean up temp file
                        temp_las_path.unlink()

                        # Report summary
                        summary = damage_report.get('summary', {})
                        total_damaged = summary.get('damaged_members', 0)
                        high_sev = summary.get('high_severity', 0)
                        med_sev = summary.get('medium_severity', 0)
                        low_sev = summary.get('low_severity', 0)
                        healthy = summary.get('healthy_members', 0)

                        self.emit_progress(f"\nDamage Detection Complete!")
                        self.emit_progress(f"  Damaged members: {total_damaged}")
                        self.emit_progress(f"    HIGH severity: {high_sev}")
                        self.emit_progress(f"    MEDIUM severity: {med_sev}")
                        self.emit_progress(f"    LOW severity: {low_sev}")
                        self.emit_progress(f"  Healthy members: {healthy}")
                        self.emit_progress(f"\nDamage report saved to: {damage_report_dir}")
                    else:
                        self.emit_progress("Warning: laspy not available, skipping damage detection")

                except Exception as e:
                    self.emit_progress(f"Warning: Damage detection failed: {str(e)}")
                    import traceback
                    self.emit_progress(traceback.format_exc())

            elif self.enable_damage_detection and not HAS_DAMAGE_DETECTION:
                self.emit_progress("\nNote: Damage detection module not available")
...
if USE_PYQT:
    class ShaftSegmentationGUI(QMainWindow):
        ...
        def init_ui(self):
            ...
            # First row: slice height and device
            row1_layout = QHBoxLayout()
            row1_layout.addWidget(QLabel("Slice Height (m):"))
            ...
            row1_layout.addWidget(self.slice_height_spin)

            row1_layout.addStretch()

            row1_layout.addWidget(QLabel("Device:"))
    ...
    class ShaftSegmentationGUI:
        ...
        def start_processing(self):
            ...
            self.processing_thread = ProcessingThread(
                ...
                enable_damage_detection=self.damage_detection_var.get()
            )
